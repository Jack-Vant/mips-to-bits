...
class Ins {
    syntax: stx;
    op_or_func: number;
    constructor(syntax: stx, op_or_func: number) {
        this.syntax = syntax;
...
class Unsolved extends Ins {
    args: string;
    ic: number;
    constructor(syntax: stx, op_or_func: number, args: string, _ic: number) {
...
function _getAddr(args: string): number | undefined {
    const lbl = PATTERNS.LBL.exec(args)?.[1];
    if (lbl == undefined) throw Error("Missing label param");
    const addr = labels[lbl];
    return addr;
}

function _getAddrOrPush(op: number, args: string, s: stx): number | undefined {
...
const INSTRUCTIONS: { [name: string]: Ins } = {
    ...
    "mthi": new Ins(jumpR, 17), // jumpR == moveTo
